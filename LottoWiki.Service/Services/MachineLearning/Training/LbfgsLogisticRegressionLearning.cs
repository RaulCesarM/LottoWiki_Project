...
{
    public class LbfgsLogisticRegressionLearning
    {
        private readonly string _modelPath;
        private readonly IDataConverter _data;

        public LbfgsLogisticRegressionLearning(IDataConverter data)
        {
            _data = data;
            _modelPath = Path.Combine("MachineLearning", "LbfgsLogisticRegression.zip");
        ...
        {
            var data = await _data.PopulateLotoFacilLbfgsLogisticRegression();
            var mlContext = new MLContext();

            IDataView dataView = mlContext.Data.LoadFromEnumerable(data);

            var splitData = mlContext.Data.TrainTestSplit(dataView, testFraction: 0.2);
            var (sdcaModel, sdcaMetrics) = EstimatorTreinners.LbfgsLogisticRegressionEstimator(mlContext, splitData);
